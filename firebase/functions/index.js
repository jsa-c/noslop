...
// Caps concurrent instances per trigger so a burst of writes can't run up
// the bill unexpectedly.
...
exports.onVoteWritten = onDocumentWritten(
  { document: "videos/{videoId}/votes/{uid}", maxInstances: MAX_INSTANCES },
  async (event) => {
...
exports.onTopViewWritten = onDocumentWritten(
  { document: "videos/{videoId}/topViews/{uid}", maxInstances: MAX_INSTANCES },
  async (event) => {
    const { videoId } = event.params;
